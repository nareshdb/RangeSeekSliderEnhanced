...
@IBDesignable open class RangeSeekSliderEnhanced: UIControl {
    ...
    public convenience init(frame: CGRect = .zero, completion: ((RangeSeekSliderEnhanced) -> Void)? = nil) {
    ...
    open weak var delegate: RangeSeekSliderEnhancedDelegate?
    ...
    /// Handle slider with custom image, you can set custom image for your handle
    @IBInspectable open var sliderLineBetweenHandlesImage: UIImage? {
        didSet {
            guard let image = sliderLineBetweenHandlesImage else {
                return
            }
            ...
            sliderLineBetweenHandles.contents = image.cgImage
        }
    }
    ...
    /// Set the height of line between handles (defaults to lineheight if not set)
    open var sliderLineBetweenHandlesHeight: CGFloat? = nil {
        didSet {
            updateLineHeight()
        }
    }
    ...
    /// Set the height of line between handles (defaults to lineheight if not set)
    open var shouldRoundCornersOfLineBetweenHandles: Bool = true {
        didSet {
            updateLineHeight()
        }
    }
    ...
    private var leftHandleAccessibilityElement: UIAccessibilityElement {
        let element: RangeSeekSliderEnhancedLeftElement = RangeSeekSliderEnhancedLeftElement(accessibilityContainer: self)
    ...
    private var rightHandleAccessibilityElement: UIAccessibilityElement {
        let element: RangeSeekSliderEnhancedRightElement = RangeSeekSliderEnhancedRightElement(accessibilityContainer: self)
        element.isAccessibilityElement = true
    ...
    /// When subclassing **RangeSeekSliderEnhanced** and setting each item in **setupStyle()**, the design is reflected in Interface Builder as well.
    ...
    private func updateLineHeight() {
        ...
        if shouldRoundCornersOfLineBetweenHandles {
            let h = sliderLineBetweenHandlesHeight ?? lineHeight
            sliderLineBetweenHandles.cornerRadius = h / 2.0
            sliderLineBetweenHandles.masksToBounds = true
        }
        else {
            sliderLineBetweenHandles.cornerRadius = 0.0
            sliderLineBetweenHandles.masksToBounds = false
        }
    }
    ...
    private func updateHandlePositions() {
        ...
        sliderLineBetweenHandles.frame = CGRect(x: leftHandle.position.x,
                                                y: sliderLine.frame.center.y - h/2,
...
// MARK: - RangeSeekSliderEnhancedLeftElement

private final class RangeSeekSliderEnhancedLeftElement: UIAccessibilityElement {

    override func accessibilityIncrement() {
        guard let slider = accessibilityContainer as? RangeSeekSliderEnhanced else { return }
...
// MARK: - RangeSeekSliderEnhancedRightElement

private final class RangeSeekSliderEnhancedRightElement: UIAccessibilityElement {
